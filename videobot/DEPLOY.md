...
Выложено **2026-08-26** (обложка Anro.AI вместо лампочки, commit `6fa6a34`): Flux Dev экшн-кадр (спорткар в дожде, искры) + Inter overlay «Anro.AI» по центру. `/start` и шапка Mini App. Рестарт **только** `videobot.service` (PID 194488 → 195294, 07:19 UTC). `uspex.service` PID 193645 и `vector.service` PID 193636 не менялись этим рестартом. `.env` и `data/` не перезаписывали (mtime `.env` 00:26:18, 1362 байт). Resume `/tmp/videobot/6748280112_resume` и 6 фото в `6748280112_photos` на месте. `/health` 200, polling `@VideobotAI777_bot`. `GET /cover.jpg` 200 (247154 байт, 1920×1080). Неподписанные POST `/api/autorolik`, `/script`, `/shoot` → 403. На прод-HTML есть `Anro.AI`. `videobot-night.timer` не ставили.
