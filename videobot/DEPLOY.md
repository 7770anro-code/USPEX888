...
Выложено **2026-08-26** (422 Kling sidecar не keep, commit `d77ee49`): в 13:11 МСК Continue не слал новый Kling — poll старого `c3.mp4.fal_id` (COMPLETED + GET /response 422) помечался `fal_keep_sidecar` и отдавал ту же ошибку. Теперь 422 validation снимает sidecar. Рестарт **только** `videobot.service` (PID 201270 → **202025**, 10:18 UTC). `uspex.service` PID 193645 и `vector.service` PID 193636 не менялись. `.env` (1362 / 00:26) и resume mtime script/c0/c2/`c3.mp4.fal_id`/n3/`wide_still_3` как были. `/health` 200, polling `@VideobotAI777_bot`. Неподписанный POST `/api/autorolik` → 403. `videobot-night.timer` не ставили.

Выложено **2026-08-26** (мёртвый Kling sidecar не блокирует WIDE fallback, commit `7312f81`): FAILED `c3.mp4.fal_id` poll+unlink, не `fal_keep_sidecar`; Kling без `@Image1`, still как `start_image`; Seedance WIDE по-прежнему с `@Image1`. Рестарт **только** `videobot.service` (PID 200808 → **201270**, 10:00 UTC). `uspex.service` PID 193645 и `vector.service` PID 193636 не менялись. `.env` (1362 / 00:26) и `data/` не перезаписывали. Resume `/tmp/videobot/6748280112_resume` mtime script/n3/c0/c2/`c3.mp4.fal_id`/m2/`wide_still_3` как были, `c3.mp4` нет. `/health` 200, polling `@VideobotAI777_bot`. Неподписанный POST `/api/autorolik` → 403. `videobot-night.timer` не ставили.

Выложено **2026-08-26** (Kling WIDE без `@Image1`, still как `start_image`, commit `e982a58`): fallback Seedance→Kling больше не шлёт токен `@Image1` (это был 422 `Invalid reference index 1`). `wide_still` уходит как `start_image_url`, как FACE-фото. Рестарт **только** `videobot.service` (PID 200283 → **200808**, 09:54 UTC). `uspex.service` PID 193645 и `vector.service` PID 193636 не менялись. `.env` (1362 / 00:26) и `data/` не перезаписывали. Resume `/tmp/videobot/6748280112_resume`: script/n0–n3/c0–c2/m0–m2/`wide_still_3.png`/`c3.mp4.fal_id` на месте, `c3.mp4` нет. `/health` 200, polling `@VideobotAI777_bot`. Неподписанный POST `/api/autorolik` → 403. `videobot-night.timer` не ставили.
