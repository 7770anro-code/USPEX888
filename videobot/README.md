...
3. **fal.ai** (дефолт, `VIDEO_PROVIDER=fal`, ключ `FAL_KEY` или `FAL_API_KEY`) — очередь `https://queue.fal.run`, заголовок `Authorization: Key $FAL_KEY`, без SDK.
   - Маршрутизация в `provider_router.ROUTING`: 1 клик / своё фото / ночь / вайб / Авторолик — **только** Kling+Seedance, без Runway. Своё фото/лицо → Kling; синтетика / общий план / ночь / Авторолик WIDE → Seedance. Файл `providers/legacy/runway_client.py` не вызывается из этого пайплайна (Act Two / явный `VIDEO_PROVIDER=runway` — отдельные места).
