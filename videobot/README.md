...
3. **Runway** `gen4.5`, хост `https://api.dev.runwayml.com`, заголовок `X-Runway-Version: 2024-11-06`.
   ...
   - Клип 1: исходное фото как `promptImage`; клипы 2+ — последний кадр предыдущего (fallback — снова исходное фото). Штатного character-consistency в API нет.
   - Одинаковый `seed` на клипы ролика (похожесть выше, не гарантия).
   - `contentModeration.publicFigureThreshold=auto`. При `FAILED` + `SAFETY.*` / лица бот пишет: «Runway отклонил это фото (политика по реальным людям)…», без ретрая.
...
- **Своё фото + текст + голос** — готовый сценарий, фото как якорь клипа 1 и fallback цепочки, выбор из 21 голоса.
