...
- **Открыть меню** — Telegram Mini App (`webapp/`), шесть категорий:
  ...
  Отдельная страница `webapp/` (HTML/JS), кнопка `web_app`. Общение: HMAC `initData` + HTTP API бота (файлы и долгие джобы). `sendData()` не используем — лимит маленького JSON и закрывает WebApp. Под карточкой одна строка-подсказка; на первом запуске и первом заходе в режим — короткий тултип. HTTPS снаружи — ngrok (`WEBAPP_PUBLIC_URL`). Без URL кнопка остаётся callback.
