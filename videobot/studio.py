...
from fal_models import fal_upscale_file, fal_virtual_tryon
from pipeline import DYNAMIC_SCENE_COUNT, PipelineError, file_to_data_uri
from presets import voice_settings_payload
from store import save_last_video, set_cloned_voice
...
async def run_studio_clone(bot: Bot, user_id: int, audio: bytes, filename: str, *, consent: bool) -> None:
    ...
    await send_chat_text(bot, user_id, "⏳ Студия: клонирую голос…")
    if BUSY.locked():
        raise PipelineError("Сейчас уже идёт другая задача. Подожди результат в чате.")
    await BUSY.acquire()
    try:
        async with aiohttp.ClientSession() as session:
            vid = await clone_voice(session, src, name=f"Клон {user_id}")
        set_cloned_voice(int(user_id), vid, "Мой голос")
        await send_chat_text(bot, user_id, "Голос клонирован. Выбери его в списке голосов при съёмке.")
    finally:
        BUSY.release()
        shutil.rmtree(work, ignore_errors=True)
...
def job_error_text(exc: BaseException) -> str:
    ...
    return "Не вышло. Попробуй ещё раз из Студии или меню бота."
